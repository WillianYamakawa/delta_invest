...
module.exports.getAll = async function(){
    ...
    SELECT clientes.id, clientes.nome, clientes.CPF, clientes.saldo, clientes.ativo, clientes.PIX, clientes.agencia_banco, clientes.conta_banco, COUNT(CASE WHEN pedidos_saque.status = 1 THEN 1 ELSE NULL END) as pedidos
    FROM clientes 
    LEFT JOIN pedidos_saque ON clientes.id = pedidos_saque.cliente_id
    GROUP BY clientes.id
    ORDER BY clientes.ativo DESC, clientes.nome
    `);
}
...
module.exports.insert = async function(opt){
    ...
    if(opt.password != undefined) opt.password = createHash('md5').update(opt.password).digest('hex'); //Hashes Passoword
    return await db.execute("INSERT INTO clientes SET ?, data_inicio_contrato=now()", opt)
}
